...
void ISHRAInt32(TYPE *A, TYPE iShiftOpnd, TYPE *C, TYPE N) {
  #define REG_CT0  31
  #define REG_CT1  30
  #define REG_DST  29
  ...
  #define REG_DST_3 21
    ...
    printf("Entered ISHRAInt32(iShiftOpnd = %d)\n", iShiftOpnd);

    // Alex: added manually
    N = N * sizeof(TYPE) / sizeof(short);
    printf("ISHRAInt32(): adjusted N = %d\n", N);
...
*/
    ...
    // See http://llvm.org/docs/LangRef.html#ashr-instruction and https://en.wikipedia.org/wiki/Arithmetic_shift
        R(REG_DST) = ISHRA(R(REG_SRC1), iShiftOpnd);
      ...
      PrintRegDebug(REG_CT1);
       ...
       EXECUTE_WHERE_EQ(
          R(REG_AUX) = ISHL(R(REG_SRC1_MSWORD), 16 - iShiftOpnd);
        PrintRegDebug(REG_AUX);
          R(REG_DST) = R(REG_SRC1) >> iShiftOpnd;
        PrintRegDebug(REG_DST);
...
int Test() {
    ...
    int iShiftOpnd = rand() % 17;

    printf("Calling ISHRAInt32 (with i32 result)...\n");
    fflush(stdout);

    ISHRAInt32(A, iShiftOpnd, C, NUM_ELEMS);
    ...
    //if (testResult == FAIL) {
        ...
        for (i = 0; i < NUM_ELEMS + 5; i++)
            if ((i < NUM_ELEMS) && ((A[i] >> iShiftOpnd) != C[i]))
                printf("C[%d] = 0x%08x ( != 0x%08x)\n", i, C[i], A[i] >> iShiftOpnd);
            else
                printf("C[%d] = %d\n", i, C[i]);
    //}
