...
void AddInt32(TYPE *A, TYPE *B, TYPE *C, TYPE N) {
  ...
  #define REG_CRY_2 21
    ...
    connexGlobal->writeDataToConnexPartial((&A [(0 +  0)]),
                    /*numVectors*/ (int)ceil(((float)(N)) / CONNEX_VECTOR_LENGTH),
                    /* actual num elems written */ (N),
                    /*offset*/ 0);
    connexGlobal->writeDataToConnexPartial((&B [(0 +  0)]), /*numVectors*/ (int)ceil(((float)(N)) / CONNEX_VECTOR_LENGTH),
                    /* actual num elems written */ (N),
    ...
    // Alex: added manually
        R(REG_CT0) = 0;
        R(REG_CT1) = 1;
        R(REG_DST) = R(REG_SRC2) + R(REG_SRC1) ; // MSA_3R generic instruction // <MCInst #27 ADDV_D


    // Alex: added manually
        /* The carry flag for adding 2 i16 (16-bit signed integer)
             numbers is actually computed, as usual, as the carry
             for 2 16-bit UNsigned numbers.
            Note: the UNsigned + and - provide the same WIDTH-bit result as
              their signed variants, BUT the carry differs.
            This carry can be used to perform 2^WIDTH-radix signed/complemented + or - by using
              the Connex + or - operations as bricks to perform e.g. 2^(2*WIDTH)-bit signed integer + or -
             (this is in a sense similar to how we compute to decimal a 2s-complement value: we consider
                all bits unsigned, except the sign bit which we use to subtract, etc).

          We simply want to retrieve the carry flag in a register,
              because we want to CELL_SHR the carry.
          The data is stored in little-endian mode:
             ...
             For all odd positions above (the most significant 16 bits of each Element),
               we add the carry of the Least significant 16 bits of the respective Element.
        */
