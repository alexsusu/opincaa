...
/*
   ...
   - with broadcast of the max value: <<number of cycles simulated = 338 (add 1 more for the REDUCE)>>
   Note: with debug messages we reach about 1000 cycles.
    ...
    Pseudocode for MAX-reduction (with halving the vector, similar to what LLVM LoopVectorize generates itself):
      This uses the fact MAX is commutative - see e.g., also http://developer.amd.com/resources/articles-whitepapers/opencl-optimization-case-study-simple-reductions/
...
int MaxReduce(TYPE *C, int N) { // TYPE N) {
    ...
    connexGlobal->writeDataToConnexPartial((&C [(0 +  0)]), /*numVectors*/ (int)ceil(((float)numElemsAccessedC) / CONNEX_VL), /* actual num elems written */ numElemsAccessedC, /*offset*/ 0); // writeDataToConnexPartial() is blocking // Generated in InstrumentVectorGatherLoadOrScatterStore()
...
// TODO TODO TODO: if necessary, do better when it is clear what instructions are predicated - see ACTIVE flag in ConnexISA.docx
    ...
    */
    ...
    connexGlobal->readCorrectReductionResults(1, &sum, sizeof(int), true);
    //printf("res = %d (0x%08x)\n", res, res);
