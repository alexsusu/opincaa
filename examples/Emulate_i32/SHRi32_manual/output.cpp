...
void SHRInt32(TYPE *A, TYPE *B, TYPE *C, TYPE N) {
  #define REG_CT0  31
  #define REG_CT1  30
  #define REG_CT16  10
  #define REG_CT31   8
    ...
    // Alex: added manually
        ...
        /*
          ...
          For the case we shift at most 16 positions:
              We put in REG_SRC1_MSBYTE the 1-position-left-shifted REG_SRC1.
              Then, we compute:
          ...
          For the case we shift more than 16 positions:
            - we put for each even lane in REG_DST the value of REG_SRC1_MSBYTE
                 SHRA by 16 - REG_SRC2.
        */
        ...
        CELL_SHR(R(REG_SRC2), R(REG_CT01)); */
  ...
  // For all odd positions with 16 < R(REG_SRC2) we make R(REG_SRC1) = 0:
      ...
      //PrintRegDebug(REG_IDXPRED2);
       ...
       EXECUTE_WHERE_EQ(
        //R(REG_SRC1) = 0; // TODO TODO: doesn't work since the value we zero out is required for REG_SRC1_MSBYTE - try harder
        // TODO: for this don't do REG_SRC2
      ...
      //PrintRegDebug(REG_CT1);
        ...
        // Computing REG_SRC1_MSBYTE ("most significant byte" of 16-bit word of
        //                              REG_SRC1 in position 0)
        CELL_SHL(R(REG_SRC1), R(REG_CT1));
        // It is required:
        NOP;
        R(REG_SRC1_MSBYTE) = SHIFT_REG;
        //PRINTREG(REG_SRC1_MSBYTE);
        PrintRegDebug(REG_SRC1_MSBYTE);
    ...
    // For all even positions with 16 < R(REG_SRC2):
      ...
      //PrintRegDebug(REG_IDXPRED);
       ...
       EXECUTE_WHERE_EQ( // For all even positions with 16 < R(REG_SRC2)
          ...
          R(REG_DST) = R(REG_SRC1_MSBYTE) >> R(REG_SRC2_16);
        PrintDebugMessage("Case > 16:");
        PrintRegDebug(REG_DST);
       );
   ...
   // For all even positions with R(REG_SRC2) <= 16:
      ...
      //PrintRegDebug(REG_SRC1_MSBYTE);
       ...
       EXECUTE_WHERE_EQ( // For all even positions
          ...
          //R(REG_AUX) = ISHL(R(REG_SRC1_MSBYTE), R(REG_SRC2_16));
          R(REG_AUX) = R(REG_SRC1_MSBYTE) << R(REG_SRC2_16);
...
int Test() {
    ...
    printf("Calling SHRInt32 (with i32 result)...\n");
    fflush(stdout);

    SHRInt32(A, B, C, NUM_ELEMS);
    ...
    //if (testResult == FAIL) {
        ...
        for (i = 0; i < NUM_ELEMS + 5; i++)
            if ((i < NUM_ELEMS) && ( (((unsigned int)A[i]) >> B[i]) != C[i]) ) {
