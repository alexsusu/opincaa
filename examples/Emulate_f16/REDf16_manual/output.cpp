...
//    #define CONTINUE_BACKUP 14
    ...
    // We now treat the special cases of f16
            ...
            PrintRegDebug(PRED2);
            R(PRED3) = R(CT1) - R(PRED2); // Note: We could also use: R(PRED3) = R(CT0) < R(SRC_MANTISSA); but we need PRED2 below also
            PrintRegDebug(PRED3);
            R(PRED3) &= R(PRED1A);
            PrintRegDebug(PRED3);
            REDUCE R(PRED3); // EXP==31 && MANTISSA!=0
            ...
            REDUCE R(PRED2A); // EXP==31 && MANTISSA==0 && SIGN==0
...
//#define OLD_WAY_OF_DOING_REDUCTION_FOR_ALL_F16 // NOT required because on the CPU we sum up mantissas on 64 bits
#ifdef OLD_WAY_OF_DOING_REDUCTION_FOR_ALL_F16
        //#define DO_RED_MIN // NOT required because on the CPU we sum up mantissas on 64 bits
        #ifdef DO_RED_MIN
            RedMin(__kernel,
        #else
            RedMax(__kernel,
...
#else // ! OLD_WAY_OF_DOING_REDUCTION_FOR_ALL_F16

        /* We avoid computing the RedMin/Max() and BroadcastScalar(), and also
         *  achieve TOTAL precision (we don't lose any bit of the mantissa -
         *   the final result is computed on the CPU on an i64) of the result
         *   by computing 6 REDUCE results for exponents in the intervals
         *   1-5, 6-10, 11-15, 16-20, 21-25, 26-30.
         *
         *   VERY IMPORTANT:
         *     - we use (signed) REDUCE because the mantissas have sign to
         *        be sum-reduced correctly
         *     - we use (signed) REDUCE because we need to use all 16 bits
         *     of the inputs, since we align the mantissas at the smallest
         *     exponent of the range. E.g., a mantissa with exponent 5,
         *     invariably of 11 bits (MSB is at index 10) needs to be SHL by
         *     5 - 1 positions (MSB will reach index 14 - VERY IMPORTANT: so it
         *     NOT the sign bit at index 15) to be aligned with the mantissas of
         *     smallest exponent 1 in the range 1..5.
         */
...
#ifdef REDUCE_IN_WHERE_NOT_WORKING_NOT_REQUIRED
...
//// 2019_01_10:            R(REF_EXP) = R(REF_EXP);
        )
        EXECUTE_IN_ALL(
...
int FloatRedTest(ConnexMachine *connex) {
  ...
  //#define NNZ 2
    ...
    */
    ...
    connex->readReductionResultsAndComputeF16(&result, 1); //&resF16);
