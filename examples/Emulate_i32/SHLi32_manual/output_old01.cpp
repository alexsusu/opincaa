...
void SHLInt32(TYPE *A, TYPE *B, TYPE *C, TYPE N) {
  #define REG_CT0  31
  #define REG_CT1  30
  #define REG_CT16  10
  #define REG_CT31   8
  ...
  #define REG_SRC2_16 21
    ...
    _BEGIN_KERNEL(BatchNumberGlobal);
     ...
     // IMPORTANT: We compute SHL.i32(R(REG_SRC1), R(REG_SRC2)) (considered to be both vectors of CONNEX_VECTOR_LENGTH/2 x i32)
    ...
    // Alex: added manually
        ...
        /*
          ...
          We do this in a somewhat unified way.
           ...
           Then we do the SHL for the REG_SRC1 - for both odd and even positions,
          ...
          For the case we shift at most 16 positions:
              We put in REG_SRC1_LSWORD the 1-position-left-shifted REG_SRC1.
              Then, we compute:
          ...
          For the case we shift more than 16 positions:
            - we put for each even lane in REG_DST the value of REG_SRC1_LSWORD
                 SHL by 16 - REG_SRC2.
        */
        ...
        /* Adapting REG_SRC2 - we propagate the 2nd operand of SHL (REG_SRC2)
...
*/
      ...
      //PrintRegDebug(REG_CT1);
        ...
        // Computing REG_SRC1_LSWORD ("least significant 16-bit word" of i32 word of
        //                              REG_SRC1 in position 0)
        CELL_SHR(R(REG_SRC1), R(REG_CT1));
        // It is required:
        NOP;
        R(REG_SRC1_LSWORD) = SHIFT_REG;
        //PRINTREG(REG_SRC1_LSWORD);
        PrintRegDebug(REG_SRC1_LSWORD);
  ...
  // Now, for all odd positions:
    // For all odd positions with 16 < R(REG_SRC2):
        R(REG_IDXMOD2) = R(REG_IDXMOD2) == R(REG_CT1);
      ...
      //PrintRegDebug(REG_IDXPRED);
       ...
       EXECUTE_WHERE_EQ( // For all odd positions with 16 < R(REG_SRC2)
          //R(REG_DST) = R(REG_AUX) | R(REG_DST);
          R(REG_SRC2_16) = R(REG_SRC2) - R(REG_CT16);
        PrintRegDebug(REG_SRC2_16);
          R(REG_DST) = R(REG_SRC1_LSWORD) << R(REG_SRC2_16);
        PrintDebugMessage("Case > 16:");
        PrintRegDebug(REG_DST);
       );

       EXECUTE_IN_ALL(
   // For all odd positions with R(REG_SRC2) <= 16:
        R(REG_AUX2) = R(REG_CT1) - R(REG_AUX2);
        R(REG_IDXPRED) = R(REG_AUX2) & R(REG_IDXMOD2);
      PrintRegDebug(REG_IDXPRED);
        R(REG_IDXPRED2) = R(REG_IDXPRED) == R(REG_CT1);
      ...
      //PrintRegDebug(REG_SRC1_LSWORD);
       ...
       EXECUTE_WHERE_EQ( // For all odd positions
          R(REG_SRC2_16) = R(REG_CT16) - R(REG_SRC2);

          //R(REG_AUX) = ISHL(R(REG_SRC1_LSWORD), R(REG_SRC2_16));
          R(REG_AUX) = R(REG_SRC1_LSWORD) >> R(REG_SRC2_16);
...
int Test() {
    ...
    printf("Calling SHLInt32 (with i32 result)...\n");
    fflush(stdout);

    SHLInt32(A, B, C, NUM_ELEMS);
