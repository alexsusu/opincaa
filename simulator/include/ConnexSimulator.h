...
class ConnexSimulator
{
    ...
    private:
        ...
        // Alex: added this for debug
        void printRegister(int index) {
            assert(index >= 0 && index < CONNEX_REG_COUNT);

            printf("R[%d] (left is index 0) = ", index);
            for (int i = 0; i < CONNEX_VECTOR_LENGTH; i++)
                //printf("%hd ", registerFile[index].getCellValue(i));
                printf("%04x ", /* We need to use unsigned to avoid at least on x86 to sign extend if the number is negative as i16*/ (unsigned short)registerFile[index].getCellValue(i));
            printf("[END]\n");
            fflush(stdout);
        }
