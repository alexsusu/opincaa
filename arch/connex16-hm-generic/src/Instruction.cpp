...
Instruction::Instruction(unsigned instruction) {
    ...
    if (type_for_opcode[opcode] != type) {
        throw string("Invalid type in Instruction::Instruction(instruction = " + to_string(instruction) + "): type = " + to_string(type) +
            " and type_for_opcode[opcode = " + to_string(opcode) + "] = " + to_string(type_for_opcode[opcode]));
    }

    // Alex: we avoid putting this code here for efficiency reasons: if (opcode == ) left = -1;
    left  = GET_LEFT(instruction);
    // Alex: we avoid putting this code here for efficiency reasons: if (opcode == CELL_SH...) dest = -1;
...
Instruction::Instruction(int opcode, int rightOrValue, int left, int dest) {
    ...
    switch (type) {
        ...
        default:
            ...
            throw string("Unknown type in Instruction::Instruction(") + std::to_string(opcode) + string(", ") + std::to_string(rightOrValue) + string(", ") + std::to_string(left) + string(", ") + std::to_string(dest) + ")";
    }

    this->left = left & ((1 << LEFT_SIZE) - 1);
    this->dest = dest & ((1 << DEST_SIZE) - 1);

    //printf("Instruction::Instruction(opcode = %d): left = %d, right = %d, dest = %d (type = %d)\n", opcode, left, right, dest, type); // Alex
}
...
string Instruction::dump() {
    ...
    switch (opcode) {
        //case _PRINT_REG: stream << "print_reg" << registerName(dest); break; // Alex
        case _PRINT_REG: stream << "PRINTREG(" << registerName(left) << ")"; break; // Alex
        case _PRINT_CHARS: stream << "PRINTCHARS((numeric value)" << value << ")"; break; // Alex
        ...
        case _BIT_REVERSE: stream << registerName(dest) << " = BITREVERSE(" << registerName(left) << ")"; break;
        ...
        case _CELL_SHL: stream << "CELL_SHL(" << registerName(left) << ", " << registerName(right) << ")"; break;
        case _CELL_SHR: stream << "CELL_SHR(" << registerName(left) << ", " << registerName(right) << ")"; break;
        ...
        case _POPCNT: stream << registerName(dest) << " = POPCNT(" << registerName(left) << ")"; break;
        ...
        case _SETLC_REDUCE: stream << "REPEAT_REDUCE(" << registerName(left) << ")"; break;
