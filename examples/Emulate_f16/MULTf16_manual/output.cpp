...
R(RND) = 0;
             ...
             Note: we use logic minimization tool Espresso (maybe we can use
              ...
              (Standard two-level logic minimization - Karnaugh maps
               ...
               Solution from Espresso is:
                  See /home/asusu/LLVM/llvm38Nov2016/llvm/build40/bin/Tests/NEW_v128i16/opincaa_standalone_apps/Emulate_f16/1Espresso/WithDCs/MULf16/Observe_zeros/espresso_MULf16_gen_2do.output
                    IS_NAN = E1_31 !M1_0 + E2_31 !M2_0 + Zero1 E2_31 + E1_31 Zero2
                    IS_INF = E1_31 + E2_31
          ...
          PrintRegDebug(IS_NAN);
            //
            R(V1) = R(E1_31) & R(ZERO2);
          PrintRegDebug(V1);
            R(V2) = R(E2_31) & R(ZERO1);
          PrintRegDebug(V2);
            R(IS_NAN) |= R(V1);
            R(IS_NAN) |= R(V2);
          PrintRegDebug(IS_NAN);
        ...
        EXECUTE_WHERE_EQ(
            // Note: R(DST) is initialized to F16_INF_POSITIVE
            R(DST) |= R(DST_SIGN);
          PrintRegDebug(DST);
...
/*
opA[0] = F16_INF_NEGATIVE; // F16 encoding for -Inf
opB[0] = F16_INF_NEGATIVE; // F16 encoding for -Inf
resCorrect[0] = F16_INF_POSITIVE;

opA[1] = F16_INF_POSITIVE; // F16 encoding for Inf
opB[1] = F16_INF_POSITIVE; // F16 encoding for Inf
resCorrect[1] = F16_INF_POSITIVE;
*/
