...
void SHRAInt32(TYPE *A, TYPE *B, TYPE *C, TYPE N) {
  ...
  #define SRC2_16 21
    ...
    _BEGIN_KERNEL(BatchNumberGlobal);
     ...
     // IMPORTANT: We compute SHRA.i32(R(SRC1), R(SRC2)) (considered to be both vectors of CONNEX_VECTOR_LENGTH/2 x i32)
    ...
    // Alex: added manually
        ...
        /*
        // We initialize these registers since they are dest regs for predicated instructions.
        R(SRC2_16) = 0;
        R(AUX) = 0;
        */
        //PrintRegDebug(CT1);
        ...
        /*
          ...
          We do this in a somewhat unified way.
            First, for all odd positions with 16 < R(SRC2) we make R(SRC2) = 16.
            We also do quite a bit of simple CSE.

           Then we do the SHRA for the SRC1 - for both odd and even positions,
          ...
          For the case we shift at most 16 positions:
              We put in SRC1_MSWORD the 1-position-left-shifted SRC1.
              Then, we compute:
                   - the SHL by 16-SRC2 of SRC1 ONLY for even positions
                        and then we OR this result with the result of a
                        SHR (SRC1) >> SRC2).

          For the case we shift more than 16 positions:
            - we put for each even lane in DST the value of SRC1_MSWORD
                 SHRA by 16 - SRC2.
        */
        ...
        R(IDXMOD2) = INDEX;
        R(IDXMOD2) = R(IDXMOD2) & R(CT1);

        /* Adapting SRC2 - we propagate the 2nd operand of SHRA (SRC2)
             in the most significant i16 word of each i32. */
        CELL_SHR(R(SRC2), R(IDXMOD2));
        // It is required:
        NOP;
        R(SRC2) = SHIFT_REG;
        PrintRegDebug(SRC2);

        R(AUX2) = R(CT16) < R(SRC2);
  /*
  // MEGA-TODO TODO: check on real Connex on Zedboard
  // For all odd positions with 16 < R(SRC2) we make R(SRC2) = 16:
        R(AUX2) = R(CT16) < R(SRC2);
        //R(AUX2) = R(CT1) - R(AUX2);
        R(IDXPRED) = R(IDXMOD2) == R(CT1);
        R(IDXPRED) = R(AUX2) & R(IDXPRED);
      //PrintRegDebug(IDXPRED);
        R(IDXPRED2) = R(IDXPRED) == R(CT1);
      //PrintRegDebug(IDXPRED2);
        NOP;
       );
       EXECUTE_WHERE_EQ(
        R(SRC2) = R(CT16);
  ...
  */
      ...
      //PrintRegDebug(CT1);
        ...
        // Computing SRC1_MSWORD ("most significant 16-bit word" of the i32 of
        //                              SRC1 in position 0)
        CELL_SHL(R(SRC1), R(CT1));
        // It is required:
        NOP;
        R(SRC1_MSWORD) = SHIFT_REG;
        //PRINTREG(SRC1_MSWORD);
        PrintRegDebug(SRC1_MSWORD);
  ...
  // Now, for all even positions:
    // For all even positions with 16 < R(SRC2):
        R(IDXMOD2) = R(IDXMOD2) == R(CT0);
        R(IDXPRED) = R(AUX2) & R(IDXMOD2);
      //PrintRegDebug(IDXPRED);
        R(IDXPRED2) = R(IDXPRED) == R(CT1);
       ...
       EXECUTE_WHERE_EQ( // For all even positions with 16 < R(SRC2)
          //R(DST) = R(AUX) | R(DST);
          R(SRC2_16) = R(SRC2) - R(CT16);
          R(DST) = SHRA(R(SRC1_MSWORD), R(SRC2_16));
        PrintDebugMessage("Case > 16:");
        PrintRegDebug(DST);
       );

       EXECUTE_IN_ALL(
   // For all even positions with R(SRC2) <= 16:
        R(AUX2) = R(CT1) - R(AUX2);
        R(IDXPRED) = R(AUX2) & R(IDXMOD2);
      PrintRegDebug(IDXPRED);
        R(IDXPRED2) = R(IDXPRED) == R(CT1);
      ...
      //PrintRegDebug(SRC1_MSWORD);
       ...
       EXECUTE_WHERE_EQ( // For all even positions
          R(SRC2_16) = R(CT16) - R(SRC2); // Note: if SRC2_16 is 16 then SHL might give error in Verilog if the 2nd operatnd is implemented with 4 bits
        PrintRegDebug(SRC2_16);

          //R(AUX) = ISHL(R(SRC1_MSWORD), R(SRC2_16));
          R(AUX) = R(SRC1_MSWORD) << R(SRC2_16);
    ...
    //connexGlobal->sdNodeVarNameRegDef[SRC1] = "nodeOpSrcCast1";
    ...
    kernel->offsetKernelToStartCodegenFrom = 9 + 1; // +1 for the END_WHERE instruction Opincaa adds automatically to each kernel
    kernel->numInstructionsToCodegen = kernel->size() - kernel->offsetKernelToStartCodegenFrom - 6 - 2;
    //kernel->numInstructionsToCodegen = 43 - 7 - 1;
