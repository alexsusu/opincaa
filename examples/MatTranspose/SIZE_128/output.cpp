...
void MatTranspose(TYPE src[SIZE][SIZE], TYPE dst[SIZE][SIZE], int N) {
    ...
    connexGlobal->writeDataToConnexPartial(src,
             /* actual num elems written */ N * N,
             /* offset */ LS_OFFSET_SRC);
  ...
  // We get just the 1st 1024 elements:
  N = 32;
...
void Test() {
  ...
  fflush(stdout);
    ...
    printf("Total time in MatMul_BTransposed() for %d runs: %d ms --> time per run = %.6f\n",
...
// printf("At[%d][%d] = %d\n");
      ...
      if (At[row][column] != A[column][row]) {
          printf("Diff: At[%d][%d] != A[%d][%d]\n",
