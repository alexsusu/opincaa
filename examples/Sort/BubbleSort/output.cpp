...
//#include "../LibMisc.h"
    ...
    #define REG_CT1             31
    #define REG_CT0             30
  // We make REG_CT1A hold all 1's except on vector index 0
    #define REG_CT1A            29
  // We make REG_CT1B hold all 1's except on vector index CONNEX_VECTOR_LENGTH - 1
    #define REG_CT1B            28
    #define REG_SRC             27
    //
    #define REG_PRED1           26
...
void BubbleSortKernel(ConnexMachine *connex, int32_t opALSIndex, int32_t resLSIndex) {
    ...
    BEGIN_KERNEL("bubblesort");
        ...
        PrintRegDebug(REG_PRED3);
          ...
          PrintDebugMessage("Doing propagation from even to odd positions at the right:");
          PrintRegDebug(REG_SRC);
            //CELL_SHL(R(REG_SRC), R(REG_CT1A));
            CELL_SHL(R(REG_SRC), R(REG_CT1B));
            NOP;
