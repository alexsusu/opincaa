...
/* VERY IMPORTANT: if we enable this macro we define the registers in the table
 assuming Connex has 64 registers. Otherwise, this macro as 32 register. */
#define REG64
...
// #define NOP_ALEX NOP; NOP; NOP; NOP;
#define NOP_ALEX
...
//    #define CONTINUE       14
  ...
  #define NEW_TREAT_SPECIAL_CASES_FP
        ...
        Applying logic minimization - we use Espresso.
          See solution at /home/asusu/LLVM/llvm38Nov2016/llvm/build40/bin/Tests/NEW_v128i16/opincaa_standalone_~Emulate_f16/1Espresso/WithDCs/ADDf16/espresso_ADDf16_gen_with_Connex_assembly.output
       ...
       // #define ISNAN 
          ...
          R(E1) = R(SRC1_EXPONENT) == R(CT31);
          R(M1) = R(SRC1_MANTISSA) == R(CT0);

          R(Sgn2) = R(SRC2_SIGN) == R(SIGN_MASK);
    #ifdef DO_SUB
          R(Sgn2) = R(Sgn2) == R(CT0);
    #endif
          R(E2) = R(SRC2_EXPONENT) == R(CT31);
          R(M2) = R(SRC2_MANTISSA) == R(CT0);
...
//            R(CONTINUE) = 0;
        ...
        EXECUTE_IN_ALL(
            ...
            PrintRegDebug(PRED1);
    // Unfortunately we don't reuse the registers from UnpackF16()
...
#endif
        opA[0] = 0x0000; // F16 encoding for (S=1,E=0x11,F=0x4c9)
//        opA[0] = 0x0001; // F16 encoding for (S=1,E=0x11,F=0x4c9)
//        opB[0] = 0x8000; // F16 encoding for (S=0,E=0x1,F=0x79a)
//        opB[0] = 0x0000; // F16 encoding for (S=0,E=0x1,F=0x79a)
        opB[0] = 0x0001; // F16 encoding for (S=0,E=0x1,F=0x79a)
        resCorrect[0] = 0x0000; // as obtained in ARM __fp16 (S=1,E=0x11,F=0x4c9)
/*
    opA[0] = 0x0400;
    opB[0] = 0x8401;
    resCorrect[0] = 0x8001;
...
//    opA[9] = 0x3D47; // F16 encoding for ...
  ...
  #define NUM_VALS 0
    ...
    for (int i = NUM_VALS; i < CONNEX_VECTOR_LENGTH; i++) {
      ...
      #ifdef SPECIAL_CASE
        resCorrect[i] = F16_NAN;
        //resCorrect[i] = F16_INF_POSITIVE;
      #else
        resCorrect[i] = 0;
      #endif
    }
    ...
    */
    ...
    printf("Calling 1st writeDataToConnex()\n");
    fflush(stdout);
    connex->writeDataToConnex(opA, 1, 0);
    printf("Calling 2nd writeDataToConnex()\n");
    fflush(stdout);
    ...
    printf("Calling connexGlobal->executeKernel()\n");
    fflush(stdout);
