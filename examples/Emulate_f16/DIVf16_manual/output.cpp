...
//    #define DST_SIGN       15
  ...
  //#define BASE_ADDRESS_softfloat_approxRecip_1k0s 900
  #define BASE_ADDRESS_softfloat_approxRecip_1k0s (CONNEX_MEM_SIZE + CONNEX_MEM_SIZE_EXTRA - 32)
  #define BASE_ADDRESS_softfloat_approxRecip_1k1s (CONNEX_MEM_SIZE + CONNEX_MEM_SIZE_EXTRA - 16)
 ...
 //#define INITIALIZE_TABLE_OF_32_F16_ENTRIES
            ...
            for (i = 0; i < 16; i++) {
                ...
                NOP;
                LS[BASE_ADDRESS_softfloat_approxRecip_1k0s + i] = R(AUX);

                R(AUX) = softfloat_approxRecip_1k1s[i];
                NOP;
                LS[BASE_ADDRESS_softfloat_approxRecip_1k1s + i] = R(AUX);
            }
  #else
 ...
 */
        ...
        EXECUTE_IN_ALL(
            ...
            R(AUX) = BASE_ADDRESS_softfloat_approxRecip_1k1s;
            R(AUX) += R(index);
            NOP;
            R(softfloat_approxRecip_1k1s_read) = LS[R(AUX)];
            PrintRegDebug(softfloat_approxRecip_1k1s_read);
            //
            R(AUX2) = BASE_ADDRESS_softfloat_approxRecip_1k0s;
            R(AUX2) += R(index);
            NOP;
            R(softfloat_approxRecip_1k0s_read) = LS[R(AUX2)];
            PrintRegDebug(softfloat_approxRecip_1k0s_read);

            R(AUX) = 0x3F;
            R(AUX) = R(SRC2_MANTISSA) & R(AUX);
            //R(softfloat_approxRecip_1k1s) * R(AUX);
        ...
        */
        PrintRegDebug(DST_MANTISSA);
        R(DST_MANTISSA) = R(DST_MANTISSA) + R(roundIncrement);
        R(DST_MANTISSA) >>= 4;
        PrintRegDebug(DST_MANTISSA);
             ...
             Note: we use logic minimization tool Espresso (maybe we can use
              ...
              (Standard two-level logic minimization - Karnaugh maps
               ...
               Solution from Espresso is:
                  See /home/asusu/LLVM/llvm38Nov2016/llvm/build40/bin/Tests/NEW_v128i16/opincaa_standalone_apps/Emulate_f16/1Espresso/WithDCs/DIVf16_TODO/espresso_DIVf16_gen.output_annotated
