...
static int connexJmpFindMatchesPass1(ConnexMachine *connex, int RunningMode,int LoadToRxBatchNumber,
                                    SiftDescriptors *SiftDescriptors1, SiftDescriptors *SiftDescriptors2)
{
    int CurrentcnxvectorChunkImg1, CurrentcnxvectorChunkImg2;
    ...
    for(CurrentcnxvectorChunkImg1 = 0; CurrentcnxvectorChunkImg1 < TotalcnxvectorChunksImg1; CurrentcnxvectorChunkImg1++)
    {
        ...
        if (RunningMode == MODE_EXECUTE_FIND_MATCHES)
        {
            ...
            //IOU_CVCI1.preWritecnxvectors(0,SiftDescriptors1->SiftDescriptorsBasicFeatures[VECTORS_CHUNK_IMAGE1*CurrentcnxvectorChunkImg1],VECTORS_CHUNK_IMAGE1);
            //if (PASS != IO_WRITE_NOW(&IOU_CVCI1)) {   printf("Writing next CurrentcnxvectorChunkImg1 to IO pipe, FAILED !"); return FAIL;}
            connex->writeDataToArray(SiftDescriptors1->SiftDescriptorsBasicFeatures[JMP_VECTORS_CHUNK_IMAGE1*CurrentcnxvectorChunkImg1], JMP_VECTORS_CHUNK_IMAGE1, 0);
            TotalIOTime += GetMilliSpan(TimeStart);
        }
        ...
        for(CurrentcnxvectorChunkImg2 = 0; CurrentcnxvectorChunkImg2 < TotalcnxvectorChunksImg2; CurrentcnxvectorChunkImg2++)
        {
            ...
            if (RunningMode == MODE_EXECUTE_FIND_MATCHES)
            {
                ...
                //if still have data, start NON_BLOCKING_IO-load cnxvector chunk on img2 to LS[364+329 ... 1023] or [364 ... 364 + 329]
                    TimeStart = GetMilliCount();
                    connex->writeDataToArray(SiftDescriptors2->SiftDescriptorsBasicFeatures[JMP_VECTORS_CHUNK_IMAGE2*CurrentcnxvectorChunkImg2],
                                             JMP_VECTORS_CHUNK_IMAGE2, JMP_VECTORS_CHUNK_IMAGE1 + UsingBuffer0or1*JMP_VECTORS_CHUNK_IMAGE2);
                    TotalIOTime += GetMilliSpan(TimeStart);
...
static ConnexMachine *staticConnex;
static int execKerneThread(int MatchingKernelNumber)
{
    staticConnex->executeKernel(std::string(TEST_PREFIX + to_string(MatchingKernelNumber)));
}

static int connexJmpFindMatchesMt(ConnexMachine *connex, int RunningMode,int LoadToRxBatchNumber,
                                    SiftDescriptors *SiftDescriptors1, SiftDescriptors *SiftDescriptors2,
                                    SiftMatches* SMs, SiftMatches* SMsFinal)
{
    int CurrentcnxvectorChunkImg1, CurrentcnxvectorChunkImg2;
    ...
    for(CurrentcnxvectorChunkImg1 = 0; CurrentcnxvectorChunkImg1 < TotalcnxvectorChunksImg1; CurrentcnxvectorChunkImg1++)
    {
        ...
        for(CurrentcnxvectorChunkImg2 = 0; CurrentcnxvectorChunkImg2 < TotalcnxvectorChunksImg2; CurrentcnxvectorChunkImg2++)
        {
            ...
            if (RunningMode == MODE_EXECUTE_FIND_MATCHES)
            {
                ...
                TimeStart = GetMilliCount();
                std::thread t1(execKerneThread, LoadToRxBatchNumber + UsingBuffer0or1);
                //connex->executeKernel(TEST_PREFIX + to_string(LoadToRxBatchNumber + UsingBuffer0or1));
                TotalBatchTime += GetMilliSpan(TimeStart);
...
static int connexJmpFindMatchesPass2(ConnexMachine *connex, int RunningMode,int LoadToRxBatchNumber,
                                    SiftDescriptors *SiftDescriptors1, SiftDescriptors *SiftDescriptors2,
                                        SiftMatches* SMs, SiftMatches* SMsFinal)
{
    int Start = GetMilliCount();
...
int test_BasicMatching_All_SSD(ConnexMachine *connex, char* fn1, char* fn2, FILE* logfile)
{
    ...
    */
    ...
    Start = GetMilliCount();
    connexJmpFindMatchesPass1(connex, MODE_CREATE_BATCHES, JMP_BASIC_MATCHING_BNR, &SiftDescriptors1, &SiftDescriptors2);
    connexJmpFindMatchesPass2(connex, MODE_CREATE_BATCHES, JMP_BASIC_MATCHING_BNR, &SiftDescriptors1, &SiftDescriptors2, &SM_ConnexArmMan2, &SM_ConnexArm2);
